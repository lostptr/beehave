...
func test_always_exexuting_all_successful_nodes() -> void:
	assert_that(sequence._execute(actor, blackboard)).is_equal(BeehaveNode.SUCCESS)
	assert_that(sequence._execute(actor, blackboard)).is_equal(BeehaveNode.SUCCESS)
...
func test_never_execute_second_when_first_is_failing() -> void:
	...
	assert_that(sequence._execute(actor, blackboard)).is_equal(BeehaveNode.FAILURE)
	assert_that(sequence._execute(actor, blackboard)).is_equal(BeehaveNode.FAILURE)
	assert_that(action1.count).is_equal(2)
...
func test_keeps_running_child_until_success() -> void:
	...
	assert_that(sequence._execute(actor, blackboard)).is_equal(BeehaveNode.RUNNING)
	assert_that(sequence._execute(actor, blackboard)).is_equal(BeehaveNode.RUNNING)
